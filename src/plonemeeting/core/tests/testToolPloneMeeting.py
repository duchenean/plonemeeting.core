...
from plonemeeting.core.etags import _modified
...
class testToolPloneMeeting(PloneMeetingTestCase):
    ...
    def test_pm_VerifyTestFiles(self):
        ...
        for path in os.sys.path:
            if 'plonemeeting.core' in path:
                pm_path = path
                break
        if not pm_path:
            raise Exception('plonemeeting.core path not found!')

        # find every plonemeeting.core test file
        saved_package = options.package
        saved_prefix = list(options.prefix)
        options.package = ['plonemeeting.core', ]
        options.prefix.append((pm_path, ''))
        pm_files_generator = find_test_files(options)
        pm_files = [f[0] for f in pm_files_generator if 'plonemeeting.core' in f[0]]
    ...
    def test_pm_ShowPloneMeetingTab(self):
        ...
        cleanRamCacheFor('plonemeeting.core.ToolPloneMeeting.showPloneMeetingTab')
