...
from plonemeeting.core.browser.itemattendee import position_type_default
from plonemeeting.core.config import AddAdvice
from plonemeeting.core.content.advice import IMeetingAdvice
...
class MeetingMigrator(CMFFolderMigrator):
    ...
    def migrate_schema_fields(self):
        ...
        if 'place' in self.used_meeting_attrs:
            ...
            vocab = get_vocab(
                self.new, "plonemeeting.core.content.meeting.places_vocabulary")
...
class Migrate_To_4200(Migrator):
    ...
    def run(self, extra_omitted=[]):
        ...
        if self.is_in_part('a'):  # main step, everything but update local roles and refresh catalog
            ...
            self.runProfileSteps('plonemeeting.core', steps=['workflow'], profile='default')
            # make sure new portal_type Meeting is installed
            self.removeUnusedPortalTypes(portal_types=['Meeting'])
            self.ps.runImportStepFromProfile('profile-plonemeeting.core:default', 'typeinfo')
