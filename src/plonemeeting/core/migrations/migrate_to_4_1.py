...
from plonemeeting.core.config import MEETING_GROUP_SUFFIXES
from plonemeeting.core.config import TOOL_FOLDER_POD_TEMPLATES
from plonemeeting.core.content.person import PMPerson
from plonemeeting.core.exportimport.content import update_labels_jar
...
from plonemeeting.core.utils import updateCollectionCriterion
...
class Migrate_To_4_1(Migrator):
    ...
    def _updateFacetedFilters(self):
        ...
        for cfg in self.tool.objectValues('MeetingConfig'):
            ...
            criteria.edit(
                'c12', **{
                    'vocabulary':
                        'plonemeeting.core.vocabularies.creatorswithnobodyforfacetedfiltervocabulary'})
            criteria.edit(
                'c23', **{
    ...
    def _adaptForPlonegroup(self):
        ...
        # extra suffixes
        from plonemeeting.core.config import EXTRA_GROUP_SUFFIXES
        ...
        for brain in self.catalog(portal_type='person'):
            ...
            if not isinstance(person, PMPerson):
                ...
                migrate_base_class_to_new_class(
                    ...
                    new_class_name='plonemeeting.core.content.person.PMPerson')
        logger.info('Done.')
    ...
    def run(self, extra_omitted=[]):
        ...
        # omit plonemeeting.core for now or it creates infinite loop as we are
        # in a plonemeeting.core upgrade step...
        self.upgradeAll(omit=['plonemeeting.core:default',
                              self.profile_name.replace('profile-', '')] + extra_omitted)

        # reinstall so versions are correctly shown in portal_quickinstaller
        # plone.app.versioningbehavior is installed
        self.reinstall(profiles=['profile-plonemeeting.core:default', ],
        ...
        if self.profile_name != 'profile-plonemeeting.core:default':
