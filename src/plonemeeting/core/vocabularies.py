...
from plonemeeting.core.config import ALL_VOTE_VALUES
...
from plonemeeting.core.utils import getAdvicePortalTypes
from plonemeeting.core.utils import number_word
from plonemeeting.core.utils import split_gender_and_number
...
class GroupsInChargeVocabulary(object):
    ...
    def __call___cachekey(method, self, context, only_selected=True, sort=True):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile('plonemeeting.core.vocabularies.groupsinchargevocabulary')
        tool = api.portal.get_tool('portal_plonemeeting')
        cfg = tool.getMeetingConfig(context)
...
class PMEveryOrganizationsVocabulary(EveryOrganizationsVocabulary):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile(
            'plonemeeting.core.vocabularies.everyorganizationsvocabulary')
        return date
...
class EveryOrganizationsAcronymsVocabulary(EveryOrganizationsVocabulary):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile(
            'plonemeeting.core.vocabularies.everyorganizationsacronymsvocabulary')
        return date
...
class CreatorsVocabulary(object):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile('plonemeeting.core.vocabularies.creatorsvocabulary')
        return date
...
class CreatorsForFacetedFilterVocabulary(object):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile(
            'plonemeeting.core.vocabularies.creatorsforfacetedfiltervocabulary')
        tool = api.portal.get_tool('portal_plonemeeting')
        cfg = tool.getMeetingConfig(context)
        return date, repr(cfg)
...
class MeetingDatesVocabulary(object):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile('plonemeeting.core.Meeting.date')
        tool = api.portal.get_tool('portal_plonemeeting')
        cfg = tool.getMeetingConfig(context)
        return date, repr(cfg)
...
class AdviceTypesVocabulary(object):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile('plonemeeting.core.vocabularies.advicetypesvocabulary')
        tool = api.portal.get_tool('portal_plonemeeting')
        cfg = tool.getMeetingConfig(context)
        return date, repr(cfg)
...
class BaseHeldPositionsVocabulary(object):
    ...
    def __call___cachekey(method,
        ...
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile(
            'plonemeeting.core.vocabularies.allheldpositionsvocabularies')
...
class SelectableItemInitiatorsVocabulary(BaseHeldPositionsVocabulary):
    ...
    def __call__(self, context):
        ...
        terms += list(get_vocab(
            context,
            'plonemeeting.core.vocabularies.detailedorganizationsvocabulary')._terms)
        return SimpleVocabulary(terms)
...
class ItemVotersVocabulary(BaseHeldPositionsVocabulary):
    ...
    def __call___cachekey(method, self, context):
        '''cachekey method for self.__call__.'''
        date = get_cachekey_volatile('plonemeeting.core.vocabularies.itemvotersvocabulary')
        # as used in a datagridfield, context may vary...
        context = get_context_with_request(context)
...
class ItemExportPDFElementsVocabulary(BaseContainedAnnexesVocabulary):
    ...
    def __call__(self, context):
        ...
        terms = get_vocab(
            context,
            'plonemeeting.core.vocabularies.'
...
# ``plonemeeting.core.MeetingItem`` so the DX schema can refer to
