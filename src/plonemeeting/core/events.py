...
from plonemeeting.core.config import ITEM_INITIATOR_INDEX_PATTERN
...
from plonemeeting.core.config import ITEM_SCAN_ID_NAME
from plonemeeting.core.config import ITEMTEMPLATESMANAGERS_GROUP_SUFFIX
from plonemeeting.core.config import MEETING_ATTENDEES_ATTRS
...
from plonemeeting.core.utils import AdviceAfterTransitionEvent
from plonemeeting.core.utils import applyOnTransitionFieldTransform
...
from plonemeeting.core.utils import ItemAfterTransitionEvent
from plonemeeting.core.utils import MeetingAfterTransitionEvent
...
def onItemTransition(item, event):
    ...
    invalidate_cachekey_volatile_for(
        'plonemeeting.core.MeetingItem.modified', get_again=True)
...
def onMeetingTransition(meeting, event):
    ...
    invalidate_cachekey_volatile_for('plonemeeting.core.Meeting.review_state', get_again=True)
...
def _invalidateOrgRelatedCachedVocabularies():
    ...
    invalidate_cachekey_volatile_for(
        "plonemeeting.core.vocabularies.everyorganizationsvocabulary", get_again=True)
    invalidate_cachekey_volatile_for(
        "plonemeeting.core.vocabularies.everyorganizationsacronymsvocabulary", get_again=True)
    invalidate_cachekey_volatile_for(
        "plonemeeting.core.vocabularies.groupsinchargevocabulary", get_again=True)
...
def _invalidateAttendeesRelatedCache(all=True, get_agains=[]):
    ...
    if all:
        ...
        invalidate_cachekey_volatile_for(
            "plonemeeting.core.browser.async.AsyncLoadItemAssemblyAndSignaturesRawFields")

    invalidate_cachekey_volatile_for(
        "plonemeeting.core.vocabularies.itemvotersvocabulary",
...
def item_added_or_initialized(item):
    ...
    invalidate_cachekey_volatile_for('plonemeeting.core.MeetingItem.modified', get_again=True)
...
def onItemModified(item, event):
    ...
    # An item has ben modified, use get_again for portlet_todo
    invalidate_cachekey_volatile_for(
        'plonemeeting.core.MeetingItem.modified',
        get_again=True)
...
def onMeetingAdded(meeting, event):
    ...
    invalidate_cachekey_volatile_for(
        'plonemeeting.core.Meeting.date', get_again=True)
    invalidate_cachekey_volatile_for(
        'plonemeeting.core.Meeting.modified', get_again=True)
    # a Meeting review_state changed
    invalidate_cachekey_volatile_for(
        'plonemeeting.core.Meeting.review_state', get_again=True)
...
def onMeetingModified(meeting, event):
    ...
    if not isinstance(event, ContainerModifiedEvent):
        ...
        if not mod_attrs or "date" in mod_attrs:
            ...
            # clean cache for "plonemeeting.core.Meeting.date"
            invalidate_cachekey_volatile_for(
                "plonemeeting.core.Meeting.date", get_again=True)
