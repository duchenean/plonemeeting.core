...
from plonemeeting.portal.browser.itemassembly import item_assembly_default
from plonemeeting.portal.browser.itemassembly import validate_item_assembly
from plonemeeting.portal.browser.itemsignatures import item_signatures_default
from plonemeeting.portal.config import ADD_SUBCONTENT_PERMISSIONS
from plonemeeting.portal.config import AddAnnex
from plonemeeting.portal.config import AddAnnexDecision
from plonemeeting.portal.config import DEFAULT_COPIED_FIELDS
from plonemeeting.portal.config import DUPLICATE_AND_KEEP_LINK_EVENT_ACTION
from plonemeeting.portal.config import EXECUTE_EXPR_VALUE
from plonemeeting.portal.config import EXTRA_COPIED_FIELDS_FROM_ITEM_TEMPLATE
from plonemeeting.portal.config import EXTRA_COPIED_FIELDS_SAME_MC
from plonemeeting.portal.config import HISTORY_COMMENT_NOT_VIEWABLE
from plonemeeting.portal.config import ITEM_DEFAULT_TEMPLATE_ID
from plonemeeting.portal.config import ITEM_MOVAL_PREVENTED
from plonemeeting.portal.config import ITEM_NO_PREFERRED_MEETING_VALUE
from plonemeeting.portal.config import NO_COMMITTEE
from plonemeeting.portal.config import NO_TRIGGER_WF_TRANSITION_UNTIL
from plonemeeting.portal.config import READER_USECASES
from plonemeeting.portal.config import SENT_TO_OTHER_MC_ANNOTATION_BASE_KEY
from plonemeeting.portal.config import WriteBudgetInfos
from plonemeeting.portal.ftw_labels.utils import get_labels
from plonemeeting.portal.indexes import previous_review_state
from plonemeeting.portal.indexes import sentToInfos
from plonemeeting.portal.content.meetingitem import MeetingItem
from plonemeeting.portal.tests.PloneMeetingTestCase import PloneMeetingTestCase
from plonemeeting.portal.tests.PloneMeetingTestCase import pm_logger
from plonemeeting.portal.tests.PloneMeetingTestCase import TestRequest
from plonemeeting.portal.tests.testUtils import ASSEMBLY_CORRECT_VALUE
from plonemeeting.portal.tests.testUtils import ASSEMBLY_WRONG_VALUE
from plonemeeting.portal.utils import fieldIsEmpty
from plonemeeting.portal.utils import get_annexes
from plonemeeting.portal.utils import get_dx_field
from plonemeeting.portal.utils import getFieldVersion
from plonemeeting.portal.utils import getTransitionToReachState
from plonemeeting.portal.utils import ON_TRANSITION_TRANSFORM_TAL_EXPR_ERROR
from plonemeeting.portal.utils import set_field_from_ajax
...
class testMeetingItem(PloneMeetingTestCase):
    ...
    def test_pm_ItemProposingGroupsVocabulary(self):
        ...
        vocab = get_vocab(
            item, "plonemeeting.portal.vocabularies.userproposinggroupsvocabulary", only_factory=True)
    ...
    def test_pm_ItemProposingGroupsWithGroupsInChargeVocabulary(self):
        ...
        vocab = get_vocab(
            None,
            "plonemeeting.portal.vocabularies.userproposinggroupswithgroupsinchargevocabulary",
            only_factory=True)
    ...
    def test_pm_SendItemToOtherMCKeptFields(self):
        ...
        self.failIf(clonedItem.budget_related)
        self.failIf(clonedItem.getBudgetInfos())
        self.failIf(clonedItem.other_meeting_configs_clonable_to)
    ...
    def _setupSendItemToOtherMC(self,
        ...
        self.do(item, 'accept')
        cleanRamCacheFor('plonemeeting.portal.content.meetingconfig.getMeetingsAcceptingItems')
    ...
    def test_pm_SendItemToOtherMCTransitionsTriggeredUntilPresented(self):
        ...
        cleanRamCacheFor('plonemeeting.portal.Meeting.getItems')
        self.assertEqual([anItem.privacy for anItem in meeting.get_items(ordered=True)],
    ...
    def test_pm_IsPrivacyViewable(self):
        ...
        self.changeUser('pmCreator1')
        cleanRamCacheFor('plonemeeting.portal.MeetingItem.isPrivacyViewable')
    ...
    def test_pm_ItemDuplicateForm(self):
        ...
        cleanRamCacheFor('plonemeeting.portal.MeetingItem.showDuplicateItemAction')
    ...
    def test_pm_ItemDuplicateFormOnlyKeepRelevantAnnexes(self):
        ...
        annex_vocab = get_vocab(
            item, u"plonemeeting.portal.vocabularies.item_duplication_contained_annexes_vocabulary")
        annex_decision_vocab = get_vocab(
            item, u"plonemeeting.portal.vocabularies.item_duplication_contained_decision_annexes_vocabulary")
    ...
    def test_pm_ItemCopyGroupsVocabulary(self):
        ...
        vocab_factory = get_vocab(
            item,
            u'plonemeeting.portal.vocabularies.itemcopygroupsvocabulary',
            only_factory=True)
    ...
    def test_pm_AssociatedGroupsVocabulary(self):
        ...
        # create an item to test the vocabulary
        item = self.create('MeetingItem')
        vocab_name = u'plonemeeting.portal.vocabularies.itemassociatedgroupsvocabulary'
    ...
    def test_pm_ItemOrderedGroupsInChargeVocabulary(self):
        ...
        # create an item to test the vocabulary
        item = self.create('MeetingItem')
        vocab_name = u'plonemeeting.portal.vocabularies.itemgroupsinchargevocabulary'
        self.assertEqual([term.token for term in get_vocab(item, vocab_name)],
                         [self.developers_uid, self.vendors_uid])
    ...
    def test_pm_ListClassifiersKeepConfigSorting(self):
        ...
        self.assertEqual([term.title for term in get_vocab(item, u'plonemeeting.portal.vocabularies.classifiersvocabulary')],
                         [u'--- Make a choice ---',
                          u'Classifier 0',
                          u'Classifier 1',
                          u'Classifier 2',
    ...
    def test_pm_OptionalAdvisersVocabulary(self):
        ...
        vocab_factory = get_vocab(
            item,
            u'plonemeeting.portal.vocabularies.itemoptionaladvicesvocabulary',
            only_factory=True)
    ...
    def test_pm_OptionalAdvisersDelayAwareAdvisers(self):
        ...
        vocab_factory_name = u'plonemeeting.portal.vocabularies.itemoptionaladvicesvocabulary'
        ...
        def _sum_entries(call_actions_panel=True):
            ...
            return sum(
                [data['entries'] for data in ramcache.getStatistics()
                 if data['path'] ==
                 'plonemeeting.portal.browser.overrides.MeetingItemActionsPanelView__call__'])
    ...
    def test_pm_Emergency(self):
        ...
        cleanRamCacheFor('plonemeeting.portal.content.meetingitem.attribute_is_used')
    ...
    def test_pm_PrintAssembly(self):
        ...
        ordered_contacts = get_vocab_values(
            cfg, 'plonemeeting.portal.vocabularies.selectableassemblymembersvocabulary')
    ...
    def test_pm_ItemEditAndView(self):
        ...
        attrs = [term.value for term in get_vocab(
            cfg, 'plonemeeting.portal.vocabularies.used_item_attributes_vocabulary',
    ...
    def test_pm_CommitteesSupplements(self):
        ...
        self._enableField("committees", related_to="Meeting")
        committees_vocab = u'plonemeeting.portal.vocabularies.item_selectable_committees_vocabulary'
        self.changeUser('pmCreator1')
        item = self.create('MeetingItem')
    ...
    def test_pm_CommitteesItemOnly(self):
        ...
        vocab = get_vocab(
            item,
            u'plonemeeting.portal.vocabularies.item_selectable_committees_vocabulary')
