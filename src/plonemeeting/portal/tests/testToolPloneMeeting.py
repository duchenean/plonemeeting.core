...
from plonemeeting.portal.etags import _modified
...
class testToolPloneMeeting(PloneMeetingTestCase):
    ...
    def test_pm_VerifyTestFiles(self):
        """
          ...
          We check that every test files in plonemeeting.portal are also in this sub-product.
        """
        # list test files from plonemeeting.portal
        ...
        for path in os.sys.path:
            if 'plonemeeting.portal' in path:
                pm_path = path
                break
        if not pm_path:
            raise Exception('plonemeeting.portal path not found!')

        # find every plonemeeting.portal test file
        saved_package = options.package
        saved_prefix = list(options.prefix)
        options.package = ['plonemeeting.portal', ]
        options.prefix.append((pm_path, ''))
        pm_files_generator = find_test_files(options)
        pm_files = [f[0] for f in pm_files_generator if 'plonemeeting.portal' in f[0]]
    ...
    def test_pm_ShowPloneMeetingTab(self):
        ...
        cleanRamCacheFor('plonemeeting.portal.ToolPloneMeeting.showPloneMeetingTab')
    ...
    def test_pm_AdvisersConfig(self):
        ...
        self.assertEqual(
            get_vocab_values(
                dev_advice,
                'plonemeeting.portal.content.advice.advice_type_vocabulary'),
