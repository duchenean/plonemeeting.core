...
from plonemeeting.portal.config import ITEM_INITIATOR_INDEX_PATTERN
...
from plonemeeting.portal.config import ITEM_SCAN_ID_NAME
from plonemeeting.portal.config import ITEMTEMPLATESMANAGERS_GROUP_SUFFIX
from plonemeeting.portal.config import MEETING_ATTENDEES_ATTRS
from plonemeeting.portal.config import MEETING_CONFIG
...
from plonemeeting.portal.config import ROOT_FOLDER
...
from plonemeeting.portal.interfaces import IMeetingContent
from plonemeeting.portal.utils import _addManagedPermissions
from plonemeeting.portal.utils import addDataChange
from plonemeeting.portal.utils import AdviceAfterAddEvent
from plonemeeting.portal.utils import AdviceAfterModifyEvent
from plonemeeting.portal.utils import AdviceAfterTransitionEvent
from plonemeeting.portal.utils import applyOnTransitionFieldTransform
...
from plonemeeting.portal.utils import ItemAfterTransitionEvent
from plonemeeting.portal.utils import MeetingAfterTransitionEvent
...
def onItemTransition(item, event):
    ...
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.MeetingItem.modified', get_again=True)
...
def onMeetingTransition(meeting, event):
    ...
    invalidate_cachekey_volatile_for('plonemeeting.portal.Meeting.modified', get_again=True)
    # invalidate last meeting review_state changed
    invalidate_cachekey_volatile_for('plonemeeting.portal.Meeting.review_state', get_again=True)
...
def _invalidateOrgRelatedCachedVocabularies():
    ...
    invalidate_cachekey_volatile_for(
        "plonemeeting.portal.vocabularies.everyorganizationsvocabulary", get_again=True)
    invalidate_cachekey_volatile_for(
        "plonemeeting.portal.vocabularies.everyorganizationsacronymsvocabulary", get_again=True)
    invalidate_cachekey_volatile_for(
        "plonemeeting.portal.vocabularies.groupsinchargevocabulary", get_again=True)
...
def _invalidateAttendeesRelatedCache(all=True, get_agains=[]):
    ...
    if all:
        ...
        invalidate_cachekey_volatile_for(
            "plonemeeting.portal.browser.async.AsyncLoadItemAssemblyAndSignaturesRawFields")

    invalidate_cachekey_volatile_for(
        "plonemeeting.portal.vocabularies.itemvotersvocabulary",
        get_again="itemvotersvocabulary" in get_agains)
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.browser.async.AsyncLoadMeetingAssemblyAndSignatures',
...
def item_added_or_initialized(item):
    ...
    invalidate_cachekey_volatile_for('plonemeeting.portal.MeetingItem.modified', get_again=True)
...
def onItemModified(item, event):
    ...
    # An item has ben modified, use get_again for portlet_todo
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.MeetingItem.modified',
        get_again=True)
...
def onMeetingAdded(meeting, event):
    ...
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.Meeting.date', get_again=True)
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.Meeting.modified', get_again=True)
    # a Meeting review_state changed
    invalidate_cachekey_volatile_for(
        'plonemeeting.portal.Meeting.review_state', get_again=True)
...
def onMeetingModified(meeting, event):
    ...
    if not isinstance(event, ContainerModifiedEvent):
        ...
        if not mod_attrs or "date" in mod_attrs:
            ...
            # clean cache for "plonemeeting.portal.Meeting.date"
            invalidate_cachekey_volatile_for(
                "plonemeeting.portal.Meeting.date", get_again=True)
