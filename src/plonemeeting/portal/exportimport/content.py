...
from plonemeeting.portal.config import MEETING_GROUP_SUFFIXES
...
from plonemeeting.portal.Extensions.imports import import_contacts
from plonemeeting.portal.profiles import DEFAULT_USER_PASSWORD
from plonemeeting.portal.utils import org_id_to_uid
from plonemeeting.portal.utils import updateCollectionCriterion
...
class ToolInitializer:
    ...
    def run(self):
        ...
        if not alreadyHaveGroups or self.data.forceAddUsersAndGroups:
            ...
            from plonemeeting.portal.config import EXTRA_GROUP_SUFFIXES
    ...
    def _finishConfigFor(self, cfg, data):
        ...
        if data.addContactsCSV:
            ...
            if IDexterityContent.providedBy(cfg):
                vocab = get_vocab(cfg, u'plonemeeting.portal.vocabularies.selectableassemblymembersvocabulary')
    ...
    def find_binary(self, path):
        """If a binary (odt, icon, photo, ...) is not found in current profile,
           try to get it from plonemeeting.portal.profiles.testing."""
        try:
            f = file(path, 'rb')
        except IOError:
            logger.warn(
                'Could not get binary at "{0}", trying to get it from the testing profile...'.format(path))
            import plonemeeting.portal
            ...
            pm_path = os.path.join(
                os.path.dirname(plonemeeting.portal.__file__),
                'profiles/testing',
                file_folder,
                filename)
            f = file(pm_path, 'rb')
...
def initializeTool(context):
    ...
    if not pqi.isProductInstalled('PloneMeeting'):
        profileId = u'profile-plonemeeting.portal:default'
