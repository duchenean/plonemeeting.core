...
from plonemeeting.portal.browser.itemchangeorder import _compute_value_to_add
from plonemeeting.portal.browser.itemchangeorder import _is_integer
from plonemeeting.portal.browser.itemchangeorder import _to_integer
from plonemeeting.portal.browser.itemchangeorder import _use_same_integer
from plonemeeting.portal.browser.itemvotes import clean_voters_linked_to
...
# from plonemeeting.portal.utils import getStatesBefore
...
from plonemeeting.portal.utils import display_as_html
from plonemeeting.portal.utils import displaying_available_items
...
from plonemeeting.portal.utils import getDateFromDelta
...
from plonemeeting.portal.utils import ItemDuplicatedFromConfigEvent
from plonemeeting.portal.utils import MeetingLocalRolesUpdatedEvent
...
class Meeting(OrderedBaseFolder, BrowserDefaultMixin):
    ...
    def setDate(self, value, **kwargs):
        ...
        if not value == current_date:
            ...
            # clean cache for "plonemeeting.portal.vocabularies.meetingdatesvocabulary"
            invalidate_cachekey_volatile_for(
                "plonemeeting.portal.vocabularies.meetingdatesvocabulary", get_again=True)
    ...
    def insertItem(self, item, forceNormal=False):
        ...
        if cfg.to_discuss_set_on_item_insert:
            item.to_discuss = toDiscussValue
            ...
            for anItem in items:
                if higherItemFound:
                    ...
                    if not insertIndexIsSubnumber:
                        anItem.item_number = itemNumber + 100
                        ...
                        anItem.item_number = itemNumber + 1
                    ...
                    anItem.item_number = itemNumber + _compute_value_to_add(itemNumber)

            if higherItemFound:
                item.item_number = insertIndex
            else:
                insertAtTheEnd = True
    ...
    def _finalize_item_insert(self, items_to_update=[]):
        ...
        cleanRamCacheFor('plonemeeting.portal.MeetingItem.getMeeting')
    ...
    def removeItem(self, item):
        ...
        # B.2.x TODO: AT schema introspection. Replace with direct DX attr clears
        # (item.item_assembly = '' etc.) once MeetingItem FTI swap (B.2.8) lands.
        ...
        for anItem in items:
            anItemNumber = anItem.item_number
            if anItemNumber > itemNumber:
                if not itemNumberIsSubnumber:
                    anItem.item_number = anItem.item_number - 100
                elif itemNumberIsSubnumber and _use_same_integer(itemNumber, anItemNumber):
                    anItem.item_number = anItem.item_number - _compute_value_to_add(anItemNumber)
        # invalidate RAMCache for MeetingItem.getMeeting
        cleanRamCacheFor('plonemeeting.portal.MeetingItem.getMeeting')
    ...
    def at_post_edit_script(self):
        ...
        invalidate_cachekey_volatile_for(
            'plonemeeting.portal.vocabularies.itemvotersvocabulary', get_again=True)
